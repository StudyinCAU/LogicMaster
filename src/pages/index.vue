...
const handleInputNotations = (value: string): string => {
  return value
    ...
    .replace(/[≡]/g, '≡')  // 添加对同或的支持
    .replace(/\s{2,}/g, ' ')
    .replace(/[^A-Za-z0-9 ¬→⇔⋀⋁⊕≡()]/g, '')  // 确保处理后的输入只包含合法字符
    .trimStart();
};
...
class VariableCount extends LogicListener {
  ...
  exitPar(ctx) {
    ...
    finalFunctionStr += `data["${ctx.getText()}"] = data["${ctx.expr().getText()}"];\n`;
  }
  ...
  exitReverse(ctx) {
    ...
    finalFunctionStr += `data["${ctx.getText()}"] = Number(!data["${ctx.expr(0).getText()}"]);\n`;
  }
  ...
  exitXor(ctx) {
    ...
    finalFunctionStr += `data["${ctx.getText()}"] = Number(data["${ctx.expr(0).getText()}"] !== data["${ctx.expr(1).getText()}"]);\n`;
  }
  ...
  exitConjunction(ctx) {
    ...
    finalFunctionStr += `data["${ctx.getText()}"] = data["${ctx.expr(0).getText()}"] && data["${ctx.expr(1).getText()}"];\n`;
  }
  ...
  exitImplication(ctx) {
    ...
    finalFunctionStr += `data["${ctx.getText()}"] = Number(data["${ctx.expr(0).getText()}"] == 0 ? 1 : data["${ctx.expr(1).getText()}"]);\n`;
  }
}
